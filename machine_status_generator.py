...
class MachineStatusGenerator:
    ...
    def __init__(self):
        ...
        self.TRACKING_DATA["StartTime"] = pd.to_datetime(
            self.TRACKING_DATA["StartTime"], format=self.TIME_FORMAT
        )
        self.TRACKING_DATA["EndTime"] = pd.to_datetime(
            self.TRACKING_DATA["EndTime"], format=self.TIME_FORMAT
        )
        self.station_number_name_mapping = {
            1: "Milling",
            2: "Polishing",
            3: "Assembly",
            4: "Conservation",
    ...
    def generate_machine_status_per_time_step(self):
        while self.current_start_time < self.END_TIME:
            self.current_time_step_end = self.current_start_time + datetime.timedelta(
                seconds=10
            )
            time_stamp = (
                self.current_start_time.strftime(self.TIME_FORMAT)
                + "-"
                + self.current_time_step_end.strftime(self.TIME_FORMAT)
            )
            self.status[time_stamp] = copy.deepcopy(self.machines)
            self.status[time_stamp]["hasDuplicate"] = False
    ...
    def add_pallets_to_status(self, current_time_step_end, time_stamp):
        current_active_pallets = self.get_filtered_tracking_status(
            self.current_start_time, current_time_step_end
        )
        for i, pallet in current_active_pallets.iterrows():
            steps = self.get_steps_for_order(pallet["order_id"])
            print(pallet["StartTime"])
            self.status[time_stamp][
                self.station_number_name_mapping[pallet["station"]]
            ]["pallets"][pallet["order_id"]] = {
    ...
    def detect_skipped_stations(self):
        ...
        while self.current_start_time < self.END_TIME:
            ...
            for station_name in self.status[time_stamp].keys():
                ...
                for pallet_id in self.status[time_stamp][station_name][
                    "pallets"
                ].keys():
                    if (
                        self.status[time_stamp][station_name]["pallets"][pallet_id][
                            "status"
                        ]
                        != "processing"
                    ):
                    ...
                    if station_number not in pallet_visited_stations[pallet_id]:
                        ...
                        if not self.check_increasing_number(
                            pallet_visited_stations[pallet_id]
                        ):
                            self.status[time_stamp][station_name]["pallets"][pallet_id][
                                "skipped_station"
        ...
        while self.current_start_time < self.END_TIME:
            ...
            for station_name in self.status[time_stamp].keys():
                for pallet_id in self.status[time_stamp][station_name][
                    ...
                    if (
                        station_name
                        not in self.status[time_stamp][station_name]["pallets"][
                            pallet_id
                        ]["needed_stations"]
                    ):
                        self.status[time_stamp][station_name]["pallets"][pallet_id][
    ...
    def add_expecting_pallets(self):
        ...
        while self.current_start_time < self.END_TIME:
            ...
            for pallet_id in pallet_prev_station.keys():
                if pallet_prev_station[pallet_id] == None:
                    continue
                current_station_name = self.station_number_name_mapping[
                    pallet_prev_station[pallet_id]
                ]
                if (
                    pallet_id
                    in self.status[time_stamp][current_station_name]["pallets"].keys()
                ):
                    continue
                next_station_number = pallet_prev_station[pallet_id] + 1
                if next_station_number not in self.station_number_name_mapping:
                    continue
                next_station_name = self.station_number_name_mapping[
                    next_station_number
                ]
                if (
                    pallet_id
                    in self.status[time_stamp][next_station_name]["pallets"].keys()
                ):
                    pallet_prev_station[pallet_id] = next_station_number
                else:
                    # add expecting pallet
                    steps = self.get_steps_for_order(pallet_id)
                    self.status[time_stamp][next_station_name]["pallets"][pallet_id] = {
                        "pallet_id": pallet_id,
            ...
            for station_name in self.status[time_stamp].keys():
                ...
                for pallet_id in self.status[time_stamp][station_name][
                    ...
                    current_time = datetime.datetime.strptime(
                        time_stamp.split("-")[0], self.TIME_FORMAT
                    )
                    processing_since = datetime.datetime.strptime(
                        self.status[time_stamp][station_name]["pallets"][pallet_id][
                            "in_station_since"
                        ],
                        self.TIME_FORMAT,
                    )
                    cum_processing_time = abs(
                        (current_time - processing_since).total_seconds()
                    )
                    over_avg_processing_time = int(cum_processing_time) > int(
                        station_processing_time.split(":")[2]
                    )
                    under_avg_processing_time = int(cum_processing_time) + 10 < int(
                        station_processing_time.split(":")[2]
                    )
                    self.status[time_stamp][station_name]["pallets"][pallet_id][
                        "too_long_in_station"
                    ] = over_avg_processing_time
                    self.status[time_stamp][station_name]["pallets"][pallet_id][
                        "too_short_in_station"
                    ] = (
                        under_avg_processing_time
                        and self.current_start_time > processing_since
                    )
            self.prev_start_time = self.current_start_time
            self.prev_end_time = self.current_time_step_end
            self.current_start_time = self.current_time_step_end
    ...
    def get_filtered_tracking_status(self, start, end):
        mask = (
            (
                (start < self.TRACKING_DATA["StartTime"])
                & (self.TRACKING_DATA["StartTime"] <= end)
            )
            | (
                (start < self.TRACKING_DATA["EndTime"])
                & (self.TRACKING_DATA["EndTime"] <= end)
            )
            | (
                (self.TRACKING_DATA["StartTime"] < start)
                & (self.TRACKING_DATA["EndTime"] > start)
            )
        )
    ...
    def check_all_pallets_unique_in_all_stations(self):
        unique = True
        for k, s in self.status.items():  # per time slot
            print(k)
            palletSet = set()
            for station in s.values():
                for p in station["pallets"].values():
                    if p["status"] != "incoming":
                        if p["pallet_id"] in palletSet:
                            unique = False
                            s["hasDuplicate"] = True
                            # print(p["pallet_id"])
                        palletSet.add(p["pallet_id"])  # add into the set
            palletSet.clear()
        return unique
...
print(generator.check_all_pallets_unique_in_all_stations())
